...
class Mask2FormerSinePositionEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "dim_t",
            torch.arange(self.num_pos_feats, dtype=torch.int64).to(torch.float32),
        )
    ...
    def forward(
        ...
        if mask is None:
            mask = self.mask.repeat(shape[0], shape[2], shape[3])
        not_mask = (~mask).to(self.mask.dtype)
        ...
        dim_t = self.temperature ** (
            self.const
            * torch.div(self.dim_t, self.const, rounding_mode="floor")
            / self.num_pos_feats
        )
...
class Mask2FormerPixelDecoderEncoderOnly(nn.Module):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        # added buffers to avoid tensors created at runtime for torchscript export
        self.register_buffer(
            "linspace_h", torch.linspace(0.5, 1024 - 0.5, 1024, dtype=torch.float32)
        )
        self.register_buffer(
            "linspace_w", torch.linspace(0.5, 1024 - 0.5, 1024, dtype=torch.float32)
        )
    ...
    def forward(
        ...
        reference_points = self.get_reference_points(
            spatial_shapes_list, valid_ratios, self.linspace_h, self.linspace_w
        )
...
class Mask2FormerPixelDecoder(nn.Module):
    ...
    def forward(
        ...
        masks = [
            self.masks.repeat(x.size(0), x.size(2), x.size(3))
